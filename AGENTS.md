...
- **Itinerario: anclas** → `recalculateItinerary()` corre después de **cada** mutación de parada y reescribe todas las fechas, sin confirmación ni undo. El walk de `computeItinerary` es un cursor contiguo salvo en las paradas con `Stop.isAnchored`, donde **salta** a la `arrivalDate` guardada. Eso es lo único que le permite al itinerario sostener un hueco (los días sin reservar entre Nápoles y Barcelona) y lo que protege a una parada con vuelo comprado de las ediciones aguas arriba. Reglas: una candidata anclada fija su propia ventana pero **nunca** mueve el cursor; el `Setting.tripStartDate` es un *cache* del ancla de la primera parada, no una fuente independiente (derivó dos meses una vez y por eso casi se reescribe el viaje entero); y si el recálculo movería una fecha existente más de `MAX_DRIFT_DAYS`, aborta y devuelve `{ error }` en vez de guardar. Antes de tocar el itinerario o después de un seed, corré **`npm run itinerary:check`**: es un dry-run contra la DB que sale 1 si algo se movería. Un itinerario sano es un punto fijo — cero drift.
